...
export let setupSkinMesh = async ({
  ...
  const createBuffer = ({ itemSize = 3, type = "vec3" }) => {
    ...
    let node = storage(attr, type as any, particleCount) as any;
  ...
  const bindMatrixNode = uniform(skinnedMesh.bindMatrix);
  ...
  let computeBone = Fn(() => {
    ...
    const boneMatX = boneMatricesNode.node.element(skinIndex.x) as any;
    const boneMatY = boneMatricesNode.node.element(skinIndex.y) as any;
    const boneMatZ = boneMatricesNode.node.element(skinIndex.z) as any;
    const boneMatW = boneMatricesNode.node.element(skinIndex.w) as any;
  ...
  material.emissiveNode = finalColor
    .mul(lifeValue.y)
    .mul(1.5)
    .mul(float(1.0).add(velocity.mul(25)))
    .mul(1);
  ...
  material.vertexNode = Fn(() => {
    ...
    const life = lifeBuffer.node.element(instanceIndex);
    const unitScale = pow(
      velocity.x.abs().add(velocity.y.abs()).add(velocity.z.abs()).add(0.0125),
      2.1,
    )
      .mul(20)
      .mul(life.y);
